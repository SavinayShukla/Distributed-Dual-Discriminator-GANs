...
def runner(rank, world_size, hprams):
    ...
    discriminator1_ = Discriminator(nz=hprams["nz"], ndf=hprams["ndf"], nc=hprams["nc"])
    discriminator2_ = Discriminator(nz=hprams["nz"], ndf=hprams["ndf"], nc=hprams["nc"])
    
    generator_.apply(weights_init)
    generator_.cuda(rank)

    discriminator1_.apply(weights_init)
    discriminator1_.cuda(rank)

    discriminator2_.apply(weights_init)
    discriminator2_.cuda(rank)
    
    generator = DDP(generator_, device_ids=[rank],output_device=rank)
    discriminator1 = DDP(discriminator1_, device_ids=[rank], output_device=rank, broadcast_buffers=False)
    discriminator2 = DDP(discriminator2_, device_ids=[rank], output_device=rank, broadcast_buffers=False)

    criterion = nn.BCELoss().cuda(rank)
    fixed_noise = torch.randn(batch_size, nz, 1, 1).cuda(rank)
    noise = torch.FloatTensor(batch_size, nz, 1, 1).cuda(rank)

    real_label = 1.
    fake_label = 0.

    optimizerD1 = optim.Adam(discriminator1.parameters(), lr=lr, betas=(0.5, 0.99))
    optimizerD2 = optim.Adam(discriminator2.parameters(), lr=lr, betas=(0.5, 0.99))
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(loader, 0):
            ...
            real_label = torch.full((batch_size,), 1, dtype=torch.float).cuda(rank)
            fake_label = torch.full((batch_size,), 0, dtype=torch.float).cuda(rank)
